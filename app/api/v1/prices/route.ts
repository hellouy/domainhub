 ...
 */
export const GET = createApiHandler("/api/v1/prices", async (request) => {
  const url = new URL(request.url)
  // 输入验证：非法值一律回退到安全默认
  const tldFilter = url.searchParams.get("tld")?.replace(/^\./, "").toLowerCase().slice(0, 63)
  const registrarFilter = url.searchParams.get("registrar")?.toLowerCase().slice(0, 63)
  if (tldFilter && !/^[a-z0-9.-]+$/.test(tldFilter)) {
  ...
  const cacheKey = `api:v1:prices:${tldFilter ?? ""}:${registrarFilter ?? ""}:${String(sortKey)}:${order}:${page}:${limit}`
