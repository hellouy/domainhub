...
async function main() {
  ...
  console.log(`registered: ${listRegisteredAdapters().length}`)
  for (const slug of ["hostingkr", "truehost"]) {
    ...
    try {
      ...
      console.log(
        `${slug}\tPASS\t${res.rawPrices.length}\t${res.strategy}\t${((Date.now() - t) / 1000).toFixed(1)}s\t${s ? `${s.tld} ${s.currency} ${s.registerPrice ?? "-"}/${s.renewPrice ?? "-"}/${s.transferPrice ?? "-"}` : ""}`,
      )
    } catch (e) {
      console.log(`${slug}\tFAIL\t${(e as Error).message.slice(0, 150)}`)
    }
  }
}
main()